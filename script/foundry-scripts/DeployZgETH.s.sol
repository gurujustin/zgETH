...
contract DeployZgETH is Script {
    function run() external {
        ...
        address newImpl = address(new ZgETH());
        console.log("ZgETH implementation deployed at: %s", newImpl);

        // upgrade proxy if on a fork
        if (isFork) {
            proxyAdmin.upgrade(ITransparentUpgradeableProxy(Addresses.ZGETH), newImpl);
        }
